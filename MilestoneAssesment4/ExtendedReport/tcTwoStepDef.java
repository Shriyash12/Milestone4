/*
 * package MS4.stepDefinition;
 * 
 * import MS4.POM.testCaseTwoPOM; import MS4.base.BaseClass; import
 * cucumber.api.java.en.Given; import cucumber.api.java.en.Then; import
 * cucumber.api.java.en.When;
 * 
 * public class tcTwoStepDef extends BaseClass { testCaseTwoPOM objTwo = new
 * testCaseTwoPOM(driver);
 * 
 * @Given("^Initialize the browser$") public void initialize_the_browser()
 * throws Throwable {
 * 
 * }
 * 
 * @Given("^Open the website$") public void open_the_website() throws Throwable
 * {
 * 
 * }
 * 
 * @When("^Website is opened click on shop product in navigation bar$") public
 * void website_is_opened_click_on_shop_product_in_navigation_bar() throws
 * Throwable {
 * 
 * }
 * 
 * @When("^under Shop Product select powder from dropdown$") public void
 * under_Shop_Product_select_powder_from_dropdown() throws Throwable {
 * 
 * }
 * 
 * @Then("^Verify if Tide free and gentle is displayed or not$") public void
 * verify_if_Tide_free_and_gentle_is_displayed_or_not() throws Throwable {
 * 
 * }
 * 
 * @Then("^close the browser$") public void close_the_browser() throws Throwable
 * {
 * 
 * }
 * 
 * @When("^Website is opened search antibacterial using search bar$") public
 * void website_is_opened_search_antibacterial_using_search_bar() throws
 * Throwable {
 * 
 * }
 * 
 * @Then("^Print the result found$") public void print_the_result_found() throws
 * Throwable {
 * 
 * }
 * 
 * @When("^Website is opened Click on learn more under How to do laundry$")
 * public void website_is_opened_Click_on_learn_more_under_How_to_do_laundry()
 * throws Throwable {
 * 
 * }
 * 
 * @Then("^Look for How to read laundry symbols from grid$") public void
 * look_for_How_to_read_laundry_symbols_from_grid() throws Throwable { // Write
 * code here that turns the phrase above into concrete actions throw new
 * PendingException(); }
 * 
 * @Then("^print the text and validate$") public void
 * print_the_text_and_validate() throws Throwable { // Write code here that
 * turns the phrase above into concrete actions throw new PendingException(); }
 * 
 * }
 */